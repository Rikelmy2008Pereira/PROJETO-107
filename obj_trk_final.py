...
def goal_track(img, bbox):
    ...
    cv2.circle(img,(c1,c2),6,(255,0,255),2)
    cv2.circle(img,(int(p1),int(p2)),5,(255,215,0),2)

    dist = math.sqrt(((c1-p1)**2)+(c2-p2)**2)
    if dist <= 10:
        cv2.putText(img,"Parabens!!",(700,100),cv2.FONT_HERSHEY_COMPLEX_SMALL,2,(205,0,200),5)

    xs.append(c1)
    ys.append(c2)

    for i in range(len(xs)-1):
        cv2.circle(img,(xs[i],ys[i]),3,(200,200,200),2)
